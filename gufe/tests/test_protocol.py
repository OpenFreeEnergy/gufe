...
class BrokenProtocol(DummyProtocol):
    def _create(
        ...
        simulations.append(
            BrokenSimulationUnit(settings=self.settings, window=21, name="problem child", initialization=alpha),
        )
...
class TestProtocolDAGResult:
    ...
    def successes(units) -> list[ProtocolUnitResult]:
        ...
        return [
            ProtocolUnitResult(source_key=u.key, inputs=u.inputs, outputs={"result": i**2}, start_time=t1, end_time=t2)
