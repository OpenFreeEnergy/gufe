...
def test_chemical_system_component_diff(solvated_complex, solvated_ligand):
    ...
    assert (
        solvated_complex.component_diff(solvated_ligand)
        == solvated_ligand.component_diff(solvated_complex)[::-1]
    )


def test_chemical_system_component_diff_incompatible_comparison(
    solvated_complex, phenol_ligand_comp
):
    with pytest.raises(
        TypeError,
        match=r"`other` must be an instance of `ChemicalSystem`, not `SmallMoleculeComponent`",
    ):
