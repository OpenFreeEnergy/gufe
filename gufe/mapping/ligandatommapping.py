...
class LigandAtomMapping(AtomMapping):
    ...
    def _ipython_display_(self, d2d=None):  # pragma: no-cover
        ...
        return display(
            Image(draw_mapping(self._compA_to_compB, self.componentA.to_rdkit(), self.componentB.to_rdkit(), d2d)),
        )
    ...
    def draw_to_file(self, fname: str, d2d=None):
        ...
        """
        data = draw_mapping(self._compA_to_compB, self.componentA.to_rdkit(), self.componentB.to_rdkit(), d2d)
        if type(data) == bytes:
            mode = "wb"
        else:
            mode = "w"
        ...
        with open(fname, mode) as f:
            f.write(draw_mapping(self._compA_to_compB, self.componentA.to_rdkit(), self.componentB.to_rdkit(), d2d))
